...
overlayGui.Name = "PC_ESP2D"
overlayGui.IgnoreGuiInset = true
overlayGui.ResetOnSpawn = false
overlayGui.DisplayOrder = 46
overlayGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
overlayGui.Parent = safeParent()

local overlayPool = {}
local overlayActive = {}

local boxZIndex = 50
local labelZIndex = 51
local tracerZIndex = 49

local bboxCornerSigns = {
    Vector3.new(-1, 1, -1),
    Vector3.new(1, 1, -1),
    Vector3.new(-1, -1, -1),
    Vector3.new(1, -1, -1),
...
local function createOverlayRecord()
...
local function acquireOverlay(player)
    local record = overlayActive[player]
    if record then
        return record
    end
    record = table.remove(overlayPool)
    if not record then
        record = createOverlayRecord()
    end
    overlayActive[player] = record
    return record
end

local function releaseOverlay(player)
    local record = overlayActive[player]
    if not record then return end
    hideOverlay(record)
    overlayActive[player] = nil
    overlayPool[#overlayPool + 1] = record
end

local function releaseAllOverlays()
    for player in pairs(overlayActive) do
...
local function getESPColor(player)
    local relation = isEnemyESP(player)
    if relation == true then
        return tintESPColor(ESP.EnemyColor)
    elseif relation == false then
        return tintESPColor(ESP.FriendColor)
    else
        return tintESPColor(ESP.NeutralColor)
    end
end

local function updateOverlayForPlayer(player, character, color)
    if not (ESP.Enabled and (ESP.BoxESP or ESP.NameHealth or ESP.Tracers)) then
        releaseOverlay(player)
        return
    end
    local cam = currentCamera()
    ...
    local success, cf, size = pcall(character.GetBoundingBox, character)
    ...
    for _, sign in ipairs(bboxCornerSigns) do
        local corner = cf:PointToWorldSpace(Vector3.new(sign.X * half.X, sign.Y * half.Y, sign.Z * half.Z))
        local viewportPoint, onScreen = cam:WorldToViewportPoint(corner)
    ...
    local overlay = acquireOverlay(player)
    local displayColor = color or Color3.fromRGB(255, 255, 255)

    if ESP.BoxESP then
        overlay.Box.Visible = true
        overlay.Box.Position = UDim2.fromOffset(minX, minY)
        overlay.Box.Size = UDim2.fromOffset(width, height)
        overlay.Stroke.Color = displayColor
        overlay.Stroke.Thickness = 2
        overlay.Stroke.Transparency = math.clamp(ESP.OutlineTransparency, 0, 1)
    else
        overlay.Box.Visible = false
    end

    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if ESP.NameHealth and humanoid then
        ...
        local labelText = string.format("%s | %d%%", nameText, percent)
        overlay.Label.Text = labelText
        overlay.Label.TextColor3 = displayColor
        local textSize = TextService:GetTextSize(labelText, overlay.Label.TextSize, overlay.Label.Font, Vector2.new(1000, 100))
        overlay.Label.Size = UDim2.fromOffset(textSize.X + 14, textSize.Y + 8)
        local labelY = minY - 6
        if labelY < textSize.Y then
            labelY = math.min(maxY + textSize.Y + 6, viewport.Y - 4)
        else
            labelY = math.max(0, labelY)
        end
        overlay.Label.Position = UDim2.fromOffset(minX + width * 0.5, labelY)
    ...
    if ESP.Tracers then
        local tracer = overlay.Tracer
        local target = Vector2.new(minX + width * 0.5, maxY)
        local origin
        ...
        if magnitude > 2 then
            ...
            tracer.Size = UDim2.fromOffset(magnitude, 2)
            tracer.Rotation = math.deg(math.atan2(delta.Y, delta.X))
            tracer.BackgroundColor3 = displayColor
        else
            tracer.Visible = false
        end
    else
...
local function espTick(p)
    ...
    updateOverlayForPlayer(p, c, color)
end
...
mkToggle(ESPP,"Box ESP", ESP.BoxESP, function(v)
    ESP.BoxESP=v
    if not v then
        releaseAllOverlays()
    end
end, "Draws 2D boxes around players detected by ESP.")
mkToggle(ESPP,"Name & Health ESP", ESP.NameHealth, function(v)
    ESP.NameHealth=v
    if not v then
        releaseAllOverlays()
    end
end, "Shows player names and health percentages above their boxes.")
mkToggle(ESPP,"Tracer Lines", ESP.Tracers, function(v)
    ESP.Tracers=v
    if not v then
        releaseAllOverlays()
    end
end, "Draws lines from your screen to tracked players for quick spotting.")
local tracerOriginCycle = mkCycle(ESPP, "Tracer Origin", {
    {label = "Screen Center", value = "Center"},
    {label = "Screen Bottom", value = "Bottom"},
}, ESP.TracerOrigin, function(val)
    ESP.TracerOrigin = val or "Center"
end, "Choose where tracer lines begin on your screen.")
...
RunService.RenderStepped:Connect(function()
    if tracerOriginCycle and tracerOriginCycle.Row then
        setInteractable(tracerOriginCycle.Row, ESP.Tracers)
    end
end)
